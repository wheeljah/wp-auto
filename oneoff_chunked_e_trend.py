"""One-off E: trend_report style 시연 (ko 4 chunks).

- style="trend" → pillar에 hook + chunks에 CTA 자동 주입
- trend: 최신 데이터/통계/뉴스 앵글 강조
- target_chunks=4 (적당히)
- 결과: HTML/JSON/log + MockWP publish

Usage:
    cd D:\\Google_blog\\wp-auto
    .venv\\Scripts\\python.exe -u oneoff_chunked_e_trend.py
"""
...
def has_hook(html: str) -> bool:
    return "wp-auto-hook" in html


def has_cta(html: str) -> bool:
    return "wp-auto-cta" in html


def main() -> None:
    t_start = time.time()
    log("===== E: trend_report style (ko, 4 chunks) =====")
    ...
    # 3. Pillar-cluster (trend style, target_chunks=4)
    chunked_gen = ChunkedContentGenerator(client, chunk_chars=300, target_chunks=4, style="trend")
    log("[3/5] Pillar-cluster (ko, style=trend, target=4)...")
    t = time.time()
    try:
    ...
    log(f"  ✓ pillar: '{cluster.pillar.title[:50]}' (has_hook={has_hook(cluster.pillar.body_html)})")
    log(f"  ✓ {len(cluster.chunks)} chunks: " + ", ".join(c.title[:20] for c in cluster.chunks))
    cta_count = sum(1 for c in cluster.chunks if has_cta(c.body_html))
    log(f"  ✓ {cta_count}/{len(cluster.chunks)} chunks have CTA")
    log(f"  ✓ cluster done in {t_cluster:.1f}s")

    # 4. Mode A: single (stitch)
    log("[4/5] Mode A: SINGLE (1 WP post)...")
    single_html = cluster.stitch_single()
    single_internal_links = count_internal_links(single_html)
    single_path = RESULTS_DIR / "E_single.html"
    single_path.write_text(single_html, encoding="utf-8")
    log(f"  ✓ single HTML: {len(single_html):,}자, internal links: {single_internal_links}, has_hook={has_hook(single_html)}, has_cta={has_cta(single_html)}, file={single_path.name}")
    single_id = None
    try:
        async def _pub_single():
            ...
            return await wp.create_draft(
                title=cluster.topic,
                content=single_html,
                slug=("e-trend-single-" + cluster.topic.replace(" ", "-"))[:50] or "e-trend-single",
    ...
    try:
        async def _pub_cluster():
            ...
            for i, spec in enumerate(cluster_specs):
                spec = dict(spec)
                spec["slug"] = f"e-trend-c-{i}-" + spec["slug"][:30]
    ...
    for i, spec in enumerate(cluster_specs):
        (RESULTS_DIR / f"E_cluster_{i+1}.html").write_text(spec["content"], encoding="utf-8")
    ...
    result = {
        ...
        "engagement": {
            "pillar_has_hook": has_hook(cluster.pillar.body_html),
            "n_chunks_with_cta": cta_count,
            "total_ctas": cta_count + (1 if has_cta(cluster.pillar.body_html) else 0),
        },
        "single_mode": {
        ...
        "html_files": {
            ...
            "cluster": [f"E_cluster_{i+1}.html" for i in range(len(cluster_specs))],
        },
    }
    result_path = RESULTS_DIR / "E_trend_result.json"
