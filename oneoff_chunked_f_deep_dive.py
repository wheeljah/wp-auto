"""One-off F: deep_dive style 시연 (ko 4 chunks).

- style="deep_dive" → 다각도 분석, 인용, 데이터, 반론 강조
- target_chunks=4
- 결과: HTML/JSON/log + MockWP publish

Usage:
    cd D:\\Google_blog\\wp-auto
    .venv\\Scripts\\python.exe -u oneoff_chunked_f_deep_dive.py
"""
...
def main() -> None:
    t_start = time.time()
    log("===== F: deep_dive style (ko, 4 chunks) =====")
    ...
    # 3. Pillar-cluster (deep_dive style, target_chunks=4)
    chunked_gen = ChunkedContentGenerator(client, chunk_chars=300, target_chunks=4, style="deep_dive")
    log("[3/5] Pillar-cluster (ko, style=deep_dive, target=4)...")
    t = time.time()
    try:
    ...
    single_path = RESULTS_DIR / "F_single.html"
    ...
    try:
        async def _pub_single():
            ...
            return await wp.create_draft(
                title=cluster.topic,
                content=single_html,
                slug=("f-deepdive-single-" + cluster.topic.replace(" ", "-"))[:50] or "f-deepdive-single",
    ...
    try:
        async def _pub_cluster():
            ...
            for i, spec in enumerate(cluster_specs):
                spec = dict(spec)
                spec["slug"] = f"f-deep-c-{i}-" + spec["slug"][:30]
    ...
    for i, spec in enumerate(cluster_specs):
        (RESULTS_DIR / f"F_cluster_{i+1}.html").write_text(spec["content"], encoding="utf-8")
    ...
    result = {
        ...
        "html_files": {
            ...
            "cluster": [f"F_cluster_{i+1}.html" for i in range(len(cluster_specs))],
        },
    }
    result_path = RESULTS_DIR / "F_deepdive_result.json"
