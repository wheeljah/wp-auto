"""One-off I: v0.6.2 모든 수정사항 통합 데모 (랜덤 뉴스 → chunked 발행).

v0.6.2 통합 기능:
- style: trend (hook + CTA 자동)
- verify_links: True (외부 링크 자동 검증)
- optimize_structure: True (TL;DR + Related + E-E-A-T footer)
- 구조: '목차' → '요약 내용', 'FAQ' → '궁금증 해결'
- 이미지 1장 자동 첨부 (자체 AI 생성)
- 한국어 한자/일본어/중국어 → 한국어 후처리

Usage:
    cd D:\\Google_blog\\wp-auto
    .venv\\Scripts\\python.exe -u oneoff_chunked_i_random_demo.py
"""
...
# v0.6.2 후처리: 외국어 한자 → 한국어 (qwen이 가끔 섞는 人物 같은 단어 정리)
CJK_REPLACEMENTS = {
    "人物": "인물",
    "代表": "대표",  # 한자 → 한국 한자어 (단, "차 대표" 같은 한국어 맥락은 그대로)
}


def clean_cjk(text: str) -> str:
    """CJK ideograph / 일본어 흔적 → 한국어."""
...
def count_features(html: str) -> dict:
    return {
        ...
        "has_faq_title": "궁금증 해결" in html,
        "has_summary_title": "요약 내용" in html,
    }


def generate_ai_image(prompt: str, output_path: Path) -> bool:
    """AI 이미지 생성 (synthesizer 호출). 실패 시 None 반환."""
    try:
        from image_synthesize_helper import synthesize  # type: ignore
        return synthesize(prompt=prompt, output_file_path=str(output_path))
    except Exception:
        # fallback: 직접 import는 안 되므로 subprocess 흉내 안 됨, 그냥 False
        return False


def main() -> None:
    t_start = time.time()
    log("===== I: v0.6.2 통합 데모 (random news → trend chunked) =====")
    ...
    log("[3/6] Pillar-cluster (ko, trend, verify+optimize ON, target=4)...")
    t = time.time()
    try:
        cluster = chunked_gen.generate_pillar_cluster(outline, language="ko", target_chunks=4)
        # v0.6.2 후처리: 한자/일본어/중국어 → 한국어
    ...
    # 4. Image 첨부 (단일 HTML용) — AI 생성
    log("[4/6] Image 생성 (AI, 자체 CC0)...")
    img_path = RESULTS_DIR / "I_image.jpg"
    img_caption = (
        "AI 생성 (자체, CC0) · {src} {date} 보도 기반 (랜덤 데모)".format(
            src=source_name, date=pub_date[:10] if pub_date and len(pub_date) >= 10 else "2026-08-03"
        )
    )
    img_ok = generate_ai_image(
        prompt=f"Editorial photojournalism image representing: {title[:80]}, news style, 16:9, high quality realistic",
        output_path=img_path,
    )
    if not img_ok:
        # fallback: 기존 1.jpg 복사 (F_heat 폭염 이미지 — 데모용)
        fallback = WP_AUTO / "oneoff" / "images" / "F_heat" / "1.jpg"
        if fallback.exists():
            img_path = WP_AUTO / "oneoff" / "results" / "I_image.jpg"
            img_path.write_bytes(fallback.read_bytes())
            img_ok = True
            log(f"  ✓ (fallback) {img_path.name} ({img_path.stat().st_size//1024}KB)")
        else:
            log(f"  ⚠ image 생성 실패, 이미지 없이 진행")
    else:
        log(f"  ✓ {img_path.name} ({img_path.stat().st_size//1024}KB)")

    # 5. Single mode
    log("[5/6] Mode A: SINGLE (1 WP post)...")
    single_html = cluster.stitch_single()
    # 이미지 삽입 (첫 chunk H2 앞)
    if img_ok:
        figure = (
            f'<figure style="margin:24px 0;text-align:center;">\n'
            f'  <img src="I_image.jpg" alt="{title[:80]} 관련 일러스트" '
            ...
            f'    <span style="display:block;margin-top:4px;">📌 <strong>출처</strong>: {img_caption}</span>\n'
            f'  </figcaption>\n'
            f'</figure>\n'
        )
    ...
    single_path = RESULTS_DIR / "I_single.html"
    ...
    try:
        async def _pub_single():
            ...
            return await wp.create_draft(
                title=cluster.topic,
                content=single_html,
                slug=("i-v6-single-" + cluster.topic.replace(" ", "-"))[:50] or "i-v6-single",
    ...
    try:
        async def _pub_cluster():
            ...
            for i, spec in enumerate(cluster_specs):
                spec = dict(spec)
                spec["slug"] = f"i-v6-c-{i}-" + spec["slug"][:30]
    ...
    for i, spec in enumerate(cluster_specs):
        (RESULTS_DIR / f"I_cluster_{i+1}.html").write_text(spec["content"], encoding="utf-8")
    ...
    result = {
        ...
        "html_files": {
            ...
            "cluster": [f"I_cluster_{i+1}.html" for i in range(len(cluster_specs))],
            "image": str(img_path.name) if img_ok else None,
        },
    }
    result_path = RESULTS_DIR / "I_v6_result.json"
