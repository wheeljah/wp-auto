"""One-off B: 영문 chunked cluster 시나리오.

시나리오:
1. Google News US RSS → random pick
2. Outline (en) → qwen2.5:3b
3. Pillar-cluster (en) — plan + N chunks + pillar
4. Mode A: single HTML → 1 WP post
5. Mode B: pillar-cluster → N+1 WP posts
6. 결과 JSON + HTML 파일 저장

Usage:
    cd D:\\Google_blog\\wp-auto
    .venv\\Scripts\\python.exe -u oneoff_chunked_b_en.py
"""
...
def main() -> None:
    ...
    try:
        outline = outline_gen.generate_outline(
            topic=title, keyword=keyword, intent="informational", length=600, language="en"
        )
    except Exception as e:
        log(f"  [ERROR] {e}")
        sys.exit(1)
    ...
    log("[3/5] Pillar-cluster (en)...")
    t = time.time()
    try:
        cluster = chunked_gen.generate_pillar_cluster(outline, language="en")
    except Exception as e:
        log(f"  [ERROR] {e}")
        sys.exit(1)
    ...
    single_path = RESULTS_DIR / "B_single.html"
    ...
    try:
        async def _pub_single():
            ...
            return await wp.create_draft(
                title=cluster.topic,
                content=single_html,
                slug=("b-en-single-" + cluster.topic.replace(" ", "-"))[:50] or "b-en-single",
    ...
    try:
        async def _pub_cluster():
            ...
            for i, spec in enumerate(cluster_specs):
                # slug 충돌 방지 prefix
                spec = dict(spec)
                spec["slug"] = f"b-en-c-{i}-" + spec["slug"][:30]
    ...
    # HTML 파일 저장 (각 cluster post)
    for i, spec in enumerate(cluster_specs):
        (RESULTS_DIR / f"B_cluster_{i+1}.html").write_text(spec["content"], encoding="utf-8")
    ...
    result = {
        ...
        "html_files": {
            ...
            "cluster": [f"B_cluster_{i+1}.html" for i in range(len(cluster_specs))],
        },
    }
    result_path = RESULTS_DIR / "B_en_result.json"
