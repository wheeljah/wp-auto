"""One-off C: 짧은 chunk (3 chunks) 시나리오.

시나리오:
1. Google News KR RSS → random pick
2. Outline (ko) → qwen2.5:3b
3. Pillar-cluster (ko) — plan을 3 subtopic으로 truncate
4. Mode A: single HTML → 1 WP post
5. Mode B: pillar-cluster → 4 WP posts (pillar + 3 chunks)
6. 결과 JSON + HTML 파일 저장

Usage:
    cd D:\\Google_blog\\wp-auto
    .venv\\Scripts\\python.exe -u oneoff_chunked_c_short.py
"""
...
from wp_auto.ai.chunked_generator import ChunkedContentGenerator, Subtopic  # noqa: E402
...
def main() -> None:
    t_start = time.time()
    log(f"===== C: Short chunk (max {MAX_CHUNKS} chunks, ko) =====")
    ...
    single_path = RESULTS_DIR / "C_single.html"
    ...
    try:
        async def _pub_single():
            ...
            return await wp.create_draft(
                title=cluster.topic,
                content=single_html,
                slug=("c-short-single-" + cluster.topic.replace(" ", "-"))[:50] or "c-short-single",
    ...
    try:
        async def _pub_cluster():
            ...
            for i, spec in enumerate(cluster_specs):
                spec = dict(spec)
                spec["slug"] = f"c-short-c-{i}-" + spec["slug"][:30]
    ...
    for i, spec in enumerate(cluster_specs):
        (RESULTS_DIR / f"C_cluster_{i+1}.html").write_text(spec["content"], encoding="utf-8")
    ...
    result = {
        ...
        "html_files": {
            ...
            "cluster": [f"C_cluster_{i+1}.html" for i in range(len(cluster_specs))],
        },
    }
    result_path = RESULTS_DIR / "C_short_result.json"
