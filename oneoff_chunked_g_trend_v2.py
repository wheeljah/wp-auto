"""One-off G: trend v2 시연 (verify_links + optimize_structure ON, ko 4 chunks).

E (trend v1)와 비교: link verification + structure optimization 추가.
- 외부 link 자동 검증 (broken → 텍스트로 변환)
- E-E-A-T footer (author + last updated + AI 검수 안내)
- TL;DR + related articles (cluster chunks)
- nut graf (첫 문장 = Why now)

Usage:
    cd D:\\Google_blog\\wp-auto
    .venv\\Scripts\\python.exe -u oneoff_chunked_g_trend_v2.py
"""
...
def main() -> None:
    t_start = time.time()
    log("===== G: trend v2 (verify_links + optimize_structure ON) =====")
    ...
    log("[3/5] Pillar-cluster (ko, trend v2, target=4)...")
    t = time.time()
    try:
    ...
    single_path = RESULTS_DIR / "G_single.html"
    ...
    try:
        async def _pub_single():
            ...
            return await wp.create_draft(
                title=cluster.topic,
                content=single_html,
                slug=("g-trendv2-single-" + cluster.topic.replace(" ", "-"))[:50] or "g-trendv2-single",
    ...
    try:
        async def _pub_cluster():
            ...
            for i, spec in enumerate(cluster_specs):
                spec = dict(spec)
                spec["slug"] = f"g-trendv2-c-{i}-" + spec["slug"][:30]
    ...
    for i, spec in enumerate(cluster_specs):
        (RESULTS_DIR / f"G_cluster_{i+1}.html").write_text(spec["content"], encoding="utf-8")
    ...
    result = {
        ...
        "html_files": {
            ...
            "cluster": [f"G_cluster_{i+1}.html" for i in range(len(cluster_specs))],
        },
    }
    result_path = RESULTS_DIR / "G_trendv2_result.json"
